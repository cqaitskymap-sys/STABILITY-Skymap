...
function matchesWindow(pull: StudyPullPoint, window: WindowFilter) {
  ...
  if (window === "overdue") return pull.status === "Overdue";
  const days = daysUntil(pull.plannedDate);
  if (window === "today") return days === 0 || pull.status === "Due Today";
...
export default function UpcomingWithdrawalsPage() {
  ...
  const studyTypes = useMemo(() => {
    const set = new Set((pulls.data || []).map((p) => p.studyType).filter(Boolean));
  ...
  }, [pulls.data, window, studyType, chamberId]);
      ...
      <Card>
        <div className="grid gap-3 border-b border-slate-100 p-4 md:grid-cols-4">
          ...
          <div className="flex items-end text-sm text-slate-500">
            As of {formatDate(todayISO())} · {filtered.length} pull point{filtered.length === 1 ? "" : "s"}
          </div>
        </div>
        ...
        {!pulls.loading && !pulls.error && paged.items.length === 0 ? (
          <EmptyState
            ...
            description="No open pull points match the selected filters."
          />
        ) : null}
              ...
              {paged.items.map((p) => (
                <div key={p.id} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                    ...
                    <p className="text-slate-500">Planned / Actual</p>
