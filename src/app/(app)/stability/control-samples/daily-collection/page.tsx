...
import { downloadBlob, formatDate, formatDateTime, paginate, toCsv, todayISO } from "@/lib/utils";
              ...
              <tbody>
                {filtered.filter((r) => r.status !== "Cancelled").map((r) => (
                  <tr key={r.id}>
                    ...
                    <td className="border border-slate-300 px-2 py-1">{formatDate(r.date)}</td>
                    ...
                    <tbody>
                      {paged.items.map((r) => (
                        <tr key={r.id} className={r.status === "Cancelled" ? "bg-slate-100 text-slate-500" : "bg-white"}>
                          ...
                          <td className="border border-slate-300 px-2 py-2 whitespace-nowrap">{formatDate(r.date)}</td>
                  ...
                  {paged.items.map((r) => (
                    <div key={r.id} className="rounded-xl border border-slate-300 bg-white p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="text-xs text-slate-500">S. No. {r.serialDisplay} · {formatDate(r.date)}</p>
